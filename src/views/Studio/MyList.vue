...
const handleFinishImageSuccess: UploadProps['onSuccess'] = (
  ...
  form.value.Finish_image = response.url
}
...
const deleteOrder = (order_id)=>{
  Fetch(`/studio/studio_order/${order_id}/`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json'
    }
  ...
  <el-table :data='order_list'>
    ...
    >
      <template #default='scope'>
        <img :src="'http://localhost:8000/'+scope.row.order_image" alt='' style='width: 100px; height: 100px' />
      </template>
    </el-table-column>
  ...
  <el-dialog v-model='dialogDom' title='Shipping address' width='500'>
    <el-form :model='form'>
      <el-form-item label='完成图片'>
        ...
        >
          <img v-if='form.Finish_image' :src='"http://localhost:8000/"+form.Finish_image' class='avatar' />
